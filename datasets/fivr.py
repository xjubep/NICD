...
class FIVR(Dataset):
    ...
    def __init__(self, args, img_root, num_negatives=None, triplet_csv=None, train_csv=None, test_csv=None,
        ...
        if self.mode == 'train_triplet':
            ...
            self.neg_paths = glob(f'{self.img_root}/distraction/*')[:5]
            self.neg_paths = [glob(f'{neg_path}/*/*.jpg') for neg_path in self.neg_paths]
            self.neg_paths = np.concatenate(self.neg_paths)
